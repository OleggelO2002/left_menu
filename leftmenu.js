...
$(()=>{setTimeout(()=>{
  ...
  $(document).click((e)=>{ 
    ...
    // Р•СЃР»Рё Р±С‹Р»Рё РѕС‚РєСЂС‹С‚С‹ СѓРІРµРґРѕРјР»РµРЅРёСЏ, С‚Рѕ РїСЂРё РєР»РёРєРµ РІРЅРµ РёС…, Р·Р°РєСЂС‹С‚СЊ РёС….
    var $target = $(e.target);
    ...
    // Р•СЃР»Рё Р±С‹Р»Рѕ СЂР°Р·РІС‘СЂРЅСѓС‚Рѕ РјРµРЅСЋ РёР· РєРѕРјРїР°РєС‚РЅРѕРіРѕ СЂРµР¶РёРјР°, С‚Рѕ, РїСЂРё РєР»РёРєРµ РІРЅРµ РјРµРЅСЋ, СЃРІРµСЂРЅСѓС‚СЊ РѕР±СЂР°С‚РЅРѕ РІ РєРѕРјРїР°РєС‚РЅС‹Р№.  
  ...
  */
  ...
  $('#gcAccountUserMenu .gc-account-user-menu').after(`
    <div class="custom-btns-wrapper">
      ...
      <a class="menutoggle-btn" href="javascript:void(0)"><span>cРІРµСЂРЅСѓС‚СЊ</span></a>
    </div>
  `);
   if(
  ...
  if($('#gcAccountUserMenu .gc-account-leftbar').length) {
    $('#gcAccountUserMenu .gc-account-leftbar')[0].addEventListener('affixed.top.stickySidebar', ()=>{
      $('#gcAccountUserMenu .gc-account-leftbar .inner-wrapper-sticky').css({'transform':''});
    });
  }

  $('#gcAccountUserMenu .gc-account-user-menu li.menu-item > a').each((i,el)=>{
    let title = $(el).attr("title");    
    title = title == "РЎР»СѓР¶Р±Р° РїРѕРґРґРµСЂР¶РєРё" ? "РЎРѕРѕР±С‰РµРЅРёСЏ" : title;
    title = $(el).parent().is('.menu-item-notifications_button_small') ? "РЈРІРµРґРѕРјР»РµРЅРёСЏ" : title;
    $(el).removeAttr("title"); 
    $(el).attr("data-title", title);    
  });

  // РЈР±РёСЂР°РµРј СЂРµРєР»Р°РјСѓ РјР°СЂР°С„РѕРЅР°
  $('#gcAccountUserMenu li.menu-item-marathon:has(a[href="https://getcourse.ru/course/marafon"])').hide();

  // Р”РѕР±Р°РІР»СЏРµРј РєР°СЃС‚РѕРјРЅС‹Рµ Р“РљС€РЅС‹Рµ СЃС‚РёР»Рё РјРµРЅСЋ
  ...
  // Fix РѕС‚СЃС‚СѓРїР° РІ СЂР°СЃСЃС‹Р»РєР°С…
  ...
  //РќРѕРІР°СЏ РјРѕР±РёР»РєР°
  if ($(window).width() <= 768) {
        $('.custom-btns-wrapper').after(`<a class="custom-btn-close-menu" href="javascript:void(0)" onclick="$('.gc-account-leftbar').removeClass('expanded')">Р—Р°РєСЂС‹С‚СЊ РјРµРЅСЋ</a>`);
  }

  //Р¤РёРєСЃ СѓРІРµРґРѕРјР»РµРЅРёР№
...
function showMobMenu(e){
    ...
    e.stopPropagation();
    e.stopImmediatePropagation(); 
    $('#gcAccountUserMenu .gc-account-leftbar').toggleClass("expanded")//.find('.gc-account-user-menu').toggle();
...
function m_custom_btn_click() {
    ...
    if(btn_action != "") {
      btn_action = btn_action.replace(/^\s*?"|^\s*?'|"\s*?$|'\s*?$/g, ''); // РѕС‡РёС‰Р°РµРј РѕС‚ РєР°РІС‹С‡РµРє РІ РЅР°С‡Р°Р»Рµ Рё РІ РєРѕРЅС†Рµ
...
function subloginMenuBtn(id){
  ...
  var CSRF = $(data).find('input[name="YII_CSRF_TOKEN"]').val();
    $.ajax({
      ...
      data: {YII_CSRF_TOKEN: CSRF, "action":"sublogin"},
      success: ()=>{
...
// РЎРєСЂС‹РІР°РµРј СЃС‡С‘С‚С‡РёРє СѓРІРµРґРѕРјР»РµРЅРёР№ РµСЃР»Рё РЅР°Р¶Р°С‚Р° РєРЅРѕРїРєР° "РћС‚РјРµС‚РёС‚СЊ РїСЂРѕС‡РёС‚Р°РЅРЅС‹РјРё"
